...
class GigsController < ApplicationController
  ...
  def gig_params
    p params
    params.require(:gig).permit(:name, :artist, :venue, :genre, :user_id, :photo, :date, :private, :latitude, :longitude, :tickets, :photo_url_two, :gig_price)
  end
  ...
  def find_random_events_array
    url = URI("https://app.ticketmaster.com/discovery/v2/events.json?apikey=dQnJo7HE3HCwNKc3HbpQvCF3ps9exT7y&locale=*&city=Madrid&classificationName=music&size=6")
    http = Net::HTTP.new(url.host, url.port)
    http.use_ssl = true
    http.verify_mode = OpenSSL::SSL::VERIFY_NONE
    request = Net::HTTP::Get.new(url)
